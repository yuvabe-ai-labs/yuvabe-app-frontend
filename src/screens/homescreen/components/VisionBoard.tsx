...
const VisionBoard: React.FC<{
  ...
  const handleAddTile = () => {
    ...
    setTiles(prev => [...prev, newTile]); // triggers useEffect → saves
  };

  const handleDelete = (id: string) => {
    setTiles(prev => prev.filter(t => t.id !== id)); // triggers save
  };

  const handleUpdateTile = (id: string, data: Partial<VisionItem>) => {
    setTiles(prev => prev.map(t => (t.id === id ? { ...t, ...data } : t))); // triggers save
  };
  ...
  const handleDragEnd = (fromIndex: number, toIndex: number) => {
    ...
    setTiles(updated); // triggers save
  };
